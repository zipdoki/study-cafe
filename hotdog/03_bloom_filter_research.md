...
-   나머지 비트에서 앞의 0의 개수 세기 → 그 레지스터의 값을 `max`로 업데이트
    

<!-- empty-paragraph -->

→ 앞의 0의 개수가 5개면 '약 2⁵ = 32개쯤 봤을 때 한 번 나올 법한 패턴'이다. 즉 레지스터는 '이 버킷에서 지금까지 본 원소들 중 가장 긴 앞의 0 개수'를 저장한다. 앞의 0이 많을수록 드문 해시값이고, 드문 값을 봤다는 건 그만큼 많은 원소를 처리했다는 증거이다. 즉 0이 많을수록 '이 값을 보기 위해 원소를 많이 처리했겠구나'라고 역추정할 수 있는 것이다.

...
이 때 HyperLogLog를 사용해 고유 유저의 수를 추정할 수 있는데, HyperLogLog는 정확도를 조금 포기하고, 아주 적은 메모리(몇 KB)로 오차 ~1% 이내로 추정하는 것이다.

HyperLogLog의 아이디어를 사용해 원소를 Set에 저장하는 대신, 해시해서 선행 0 개수만 레지스터에 기록하는 방법을 사용하면 적은 메모리로도 이를 추정할 수 있다.
...
직관: 선행 0이 5개면 '약 2⁵ = 32개쯤 봤을 때 한 번 나올 법한 패턴'. 즉 레지스터는 '이 버킷에서 가장 드문 사건의 희귀도'를 저장한다.
